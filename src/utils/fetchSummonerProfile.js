...
//   try {
//     const response = await fetch("https://ddragon.leagueoflegends.com/cdn/15.8.1/data/en_US/summoner.json");
...
// const fetchLiveGame = async (regionCode, puuid) => {
//   try {
//     const { platform, routing } = REGION_ROUTING[regionCode];
//     console.log(`Fetching live game for puuid: ${puuid} in region: ${regionCode}`);
    
//     const liveGameUrl = `https://${platform}.api.riotgames.com/lol/spectator/v5/active-games/by-summoner/${puuid}`;
//     console.log("Live game URL:", liveGameUrl);
    
//     const liveGameData = await fetchWithAuth(liveGameUrl);
//     console.log("Live game data received:", liveGameData);
    
//     // Get summoner spells mapping
//     const spellsMap = await fetchSummonerSpells();
    
//     // Process participant data
//     const participants = await Promise.all(
//       liveGameData.participants.map(async (participant) => {
//         // Get ranked data for each participant
//         try {
//           const rankedEntries = await fetchWithAuth(
//             `https://${platform}.api.riotgames.com/lol/league/v4/entries/by-summoner/${participant.summonerId}`
//           );
          
//           const soloRank = rankedEntries.find((e) => e.queueType === "RANKED_SOLO_5x5");
...
//             spell1Id: spellsMap[participant.spell1Id.toString()] || "SummonerFlash",
//             spell2Id: spellsMap[participant.spell2Id.toString()] || "SummonerFlash",
//             perks: {
//               primaryRuneId: participant.perks.perkStyle,
//               subStyleId: participant.perks.perkSubStyle,
//             },
//             rank: soloRank ? {
//               tier: soloRank.tier,
//               rank: soloRank.rank,
...
//           };
//         } catch (error) {
//           console.error(`Error fetching rank for participant ${participant.summonerName}:`, error);
//           return {
//             summonerId: participant.summonerId,
//             summonerName: participant.summonerName,
//             championId: participant.championId,
...
//       gameType: liveGameData.gameType,
//       gameStartTime: liveGameData.gameStartTime,
//       mapId: liveGameData.mapId,
//       gameLength: liveGameData.gameLength,
...
// const fetchSummonerProfile = async (regionCode, input, liveGameOnly = false) => {
//   try {
...
//     console.log(`Fetching account data for ${gameName}#${tagLine} in ${regionCode}`);
...
//     // Return minimal data if we're only refreshing live game
//     // if (liveGameOnly) {
//     //   const liveGameData = await fetchLiveGame(regionCode, puuid);
...
//     console.log(`Fetching ranked data for summoner id: ${summoner.id}`);
//     const rankedEntries = await fetchWithAuth(
...
//     console.log(`Fetching ranked data for ${allSummonerIds.size} players from matches`);
//     await Promise.all(
//       Array.from(allSummonerIds).map(async (id) => {
//         try {
//           const ranks = await fetchWithAuth(
//             `https://${platform}.api.riotgames.com/lol/league/v4/entries/by-summoner/${id}`
//           );
//           const soloRank = ranks.find((e) => e.queueType === "RANKED_SOLO_5x5");
//           playerRankMap[id] = soloRank
//             ? { tier: soloRank.tier, rank: soloRank.rank }
//             : { tier: "Unranked", rank: "" };
//         } catch (error) {
...
//     console.log("Generating champion statistics");
//     const championMap = {};
...
const fetchWithAuth = (url) =>
  fetch(url, { headers: { "X-Riot-Token": API_KEY } }).then((res) => {
